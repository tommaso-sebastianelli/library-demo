...
export interface LoginOpt {
	...
	// Adds 'profile', 'email' and 'openid' to the requested scopes. True if unspecified.

	hosted_domain?: string;
	// The G Suite domain to which users must belong to sign in.
	// This is susceptible to modification by clients, so be sure to verify the hosted domain property of the returned user.
	// Use GoogleUser.getHostedDomain() on the client, and the hd claim in the ID Token on the server to verify the domain
	// is what you expected.

	openid_realm?: string;
	// Used only for OpenID 2.0 client migration.
	// Set to the value of the realm that you are currently using for OpenID 2.0, as described in OpenID 2.0 (Migration).
	...
	redirect_uri?: string;
	// If using ux_mode='redirect', this parameter allows you to override the default redirect_uri that will be used at the
	// end of the consent flow.
